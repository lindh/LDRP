	...
	def step(self, joint_action):
		...
		while do:
			...
			for i in range(self.agent_num):
				#act8, when the destination is the same as another agent's 
				#When self is on a node
				if self.current_goal[i] == None:
					for j in range(self.agent_num):
						if j != i and joint_action[i] == joint_action[j]: 
							joint_action[i] = self.current_start[i] 
							do = True #The condition may change, so loop once more
							break

				#act9, head-on collision
				#When self is on a node
				if self.current_goal[i] == None:
					for j in range(self.agent_num):
